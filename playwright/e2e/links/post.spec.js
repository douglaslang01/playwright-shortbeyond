...
test.describe('POST /api/links', () => {
    ...
    test('não deve encurtar quando o título original não é informada', async ({ links }) => {
    ...
    test('não deve encurtar quando a url original é inválida', async ({ links }) => {
        const response = await links.createLink({ ...user.link, original_url: 'douglas.lang@gmail.com' }, token);
